...
It **observes** existing operational state only. It does not issue tickets, generate PDFs or wallet bytes, alter recovery state, enqueue work, or expose signing secrets. Vendor **Event Studio** operational capability authoring (`field_mel_op_capabilities`) is separate: configuration metadata only, not consumed as live diagnostics input unless explicitly integrated in a future phase (see [vendor-operational-capability-studio.md](./vendor-operational-capability-studio.md)).
