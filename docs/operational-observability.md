...
- Duplicating QR signing, payload shaping, entitlement normalization, **capability policy**, **venue gate / replay scaffolding**, **operational timing windows**, or **session / sequencing / exhaustion semantics** outside **`TicketQrPayload`**, **`UniversalTicketViewModelBuilder`**, **`TicketCapabilityManager`**, **`TicketIssuer`**, **`EntitlementCapabilityRegistry`**, **`TimedEntryPolicyManager`**, **`SessionEntitlementPolicyManager`**, and **`VenueOperationPolicyManager`** (use the registry for immutable type maps; use the venue policy manager for gate execution metadata and staff-side integrity envelopes; use the timed entry manager for clock policy; use the session entitlement manager for operational session orchestration)
