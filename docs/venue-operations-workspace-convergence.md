...
| `OperationalCoordinationStateManager` (`myeventlane_tickets.operational_coordination_state_manager`) | Canonical coordination / posture / confidence / saturation normalization; composes read-models from merged rollups + existing escalation/resolution projections only (no scanner or entitlement mutation). |
| `OperationalCoordinationProjectionBuilder` (`myeventlane_tickets.operational_coordination_projection_builder`) | Coordination workspace card sections (pre-shaped labels/values). |
| `OperationalCoordinationAuditProjector` (`myeventlane_tickets.operational_coordination_audit_projector`) | Coordination audit timelines and history summaries without secrets or customer identifiers. |
| `FulfillmentLifecycleManager` (`myeventlane_tickets.fulfillment_lifecycle_manager`) | Canonical fulfillment lifecycle normalization; consumes merged inspector rollups + entitlement capabilities + continuity timing refs (read-model only). |
| `FulfillmentExecutionProjectionBuilder` (`myeventlane_tickets.fulfillment_execution_projection_builder`) | Fulfillment execution, lifecycle tally, pickup/redemption visibility cards for the workspace when **`govern mel fulfillment lifecycle`** is present. |
| `FulfillmentAuditProjector` (`myeventlane_tickets.fulfillment_audit_projector`) | Fulfillment audit timelines and summaries without replay/QR/device/customer-sensitive payloads. |
...
| `venue_operations_workspace` Twig template | Renders pre-shaped `sections` and `meta` only — **no policy branching, no calculations**. Timeline rows support `lane_state` (coordination audit) with fallback to `ownership_state` (ownership audit). |

**Composition note:** `OperationalIntegrityInspector` already orchestrates `TicketCapabilityManager`, `TimedEntryPolicyManager`, `SessionEntitlementPolicyManager`, `ZoneAccessPolicyManager`, `DeviceOperationIdentityManager`, `OperationalContinuityPolicyManager`, and `OccupancyPolicyManager` for `inspectOrder()` output. The workspace builder **reuses** that spine and adds only **type-catalog** rows via `EntitlementCapabilityRegistry` + `VenueOperationPolicyManager::describeEntitlementGateSemantics()` so policy is not evaluated twice for the same ticket rows. **Governance note (Phase 3A Commit 4):** governed severity is **projected** from merged rollup machine strings via `OperationalEscalationPolicyManager::projectSeverityFromOperationalRollup()` — not a second scanner or continuity evaluation. **Coordination note (Phase 3B Commit 1):** coordination state consumes the same merged rollups plus governed severity and resolution projection via `OperationalCoordinationStateManager` — see [operational-coordination-state-convergence.md](./operational-coordination-state-convergence.md). **Fulfillment note (Phase 4A):** lifecycle, pickup, redemption, partial fulfillment, and fulfillment audit visibility are composed when permitted — see [fulfillment-lifecycle-convergence.md](./fulfillment-lifecycle-convergence.md).
...
- Controllers or Twig computing escalation, SLA deadlines, suppression outcomes, resolution transitions, coordination state, posture, confidence, or saturation outside the governance / coordination managers and their projectors.
