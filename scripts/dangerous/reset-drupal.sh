#!/bin/bash
#
# DANGEROUS — wipes web/sites/default/files/sync and re-exports from DB.
#
# Safe environment: local DDEV only when sync dir is intentionally disposable.
# Destructive: YES — deletes all files in the active sync directory.
# Required confirmation: commit or back up config/sync and export-db before running.
#
set -e
