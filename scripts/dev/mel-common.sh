...
mel_allocate_worktree_ports() {
  # Stable port trio in 59200–59899 based on slug hash (avoids main 59000 and wallet 59100).
  local slug="$1"
  local hash base
  hash="$(printf '%s' "${slug}" | cksum | awk '{print $1}')"
  base=$((59200 + (hash % 230) * 3))
  printf '%s %s %s' "${base}" "$((base + 1))" "$((base + 2))"
}
...
$(mel_allocate_worktree_ports "${slug}")
EOF
