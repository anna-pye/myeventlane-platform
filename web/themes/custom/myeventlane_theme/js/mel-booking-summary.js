...
(function (Drupal, drupalSettings, once) {
  ...
  function getTicketRows(form) {
    return Array.from(form.querySelectorAll('.mel-ticket-row[data-ticket-price-number]'));
  }
  ...
  function getQtyInput(row) {
    return row.querySelector('input.mel-ticket-quantity, input[type="number"]');
  }
  ...
  function getTargets(form) {
    const root = form.closest('.mel-booking-v2') || document;
    return {
      container: root,
  ...
  function bindForm(form, settings) {
    ...
    const sync = () => {
      ...
      rows.forEach((row) => {
        ...
        row.setAttribute('data-selected', qty > 0 ? 'true' : 'false');
      });
  ...
  Drupal.behaviors.melBookingSummary = {
    attach(context) {
      const settings = drupalSettings.melBookingSummary;
