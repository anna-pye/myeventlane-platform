...
/**
 * Queue worker for 24-hour event reminder notifications.
 *
 * @QueueWorker(
 *   id = "event_reminder_24h",
 *   title = @Translation("24-hour event reminder"),
 *   cron = {"time" = 60}
 * )
 */
final class EventReminder24hWorker extends QueueWorkerBase implements ContainerFactoryPluginInterface {
  ...
  public function processItem($data): void {
    ...
    $context = $this->buildEmailContext($order, $event, '24 hours');
    ...
    // Send email.
    try {
      $this->messagingManager->queue('event_reminder_24h', $orderEmail, $context, [
      ...
      $this->logger->info('Sent 24-hour reminder for order @order_id, event @event_id to @email', [
        '@order_id' => $orderId,
        '@event_id' => $eventId,
        '@email' => $orderEmail,
      ]);
